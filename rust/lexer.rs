...
// Identifier/keywords
fn lex_identifier(stream: &mut Stream) -> TokenType {
...
// Strings
fn lex_str(stream: &mut Stream, end: char) -> TokenType {
    let mut chr = get_chr(stream);
    ...
    while chr != end {
        if chr == '\0' {
            ...
            return TokenType::Error(
                format!("unterminated string, missing {}", end).to_string()
            );
        }
        ret.push(chr);
        chr = next_chr(stream);
...
// Numbers
fn lex_number(stream: &mut Stream) -> TokenType {
    let mut chr = get_chr(stream);
    let mut num = "".to_string();
    // Get the digits as a string
    while chr.is_digit(10) {
        num.push(chr);
        chr = next_chr(stream);
    }
    if chr != '.' {
...
fn get_tok(stream: &mut Stream) -> TokenType {
    ...
    return match chr {
        ...
        '\'' | '"' => lex_str(stream, chr),
        // Numbers
        '0'..='9' => {stream.at -= 1; lex_number(stream)},
        // Whitespace/comments
        ' ' | '\t' | '\n' | '\r' | '\0' => TokenType::Blank,
        '#' => {
...
pub fn lex(str: &String, name: String) -> Vec<Token> {
    let mut stream = Stream{
        str: "".to_string(), name: name,
        line: 0, at: 0
    };
    ...
    for line in str.lines() {
        ...
        while get_chr(&stream) != '\0' {
            ...
            let token = get_tok(&mut stream);
            if let TokenType::Error(msg) = token.clone() {
                errors = true;
                err(&stream, &msg, ErrType::Err);
            }
            if let TokenType::Blank = token.clone() {
                continue;
            }
            if !errors {
                ret.push(Token{stream: old_stream, token: token});
            }
        }
    }
    // Return
